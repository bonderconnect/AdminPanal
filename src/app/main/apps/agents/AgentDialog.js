...
import { closeDialog, createAgent, updateAgent } from './store/agentDialog';
import { getAgents } from './store/agents';
...
function AgentDialog(props) {
	...
	const {
		...
		errors: storeErrors
	} = useSelector(({ agentsApp }) => agentsApp.agentDialog);
	...
	const callback = () => {
		if (type === 'new') {
			dispatch(showMessage({ message: 'Agent created successfully' }));
		} else {
			dispatch(showMessage({ message: 'Agent updated successfully' }));
		}
		dispatch(getAgents());
	};
	...
	const onSubmit = (data, e) => {
		...
		if (type === 'new') {
			dispatch(createAgent({ form: data, callback }));
		} else {
			dispatch(updateAgent({ form: data, callback }));
	...
	return (
		<Dialog fullWidth maxWidth="xs" aria-labelledby="new-agent-dialog-title" open={open} onClose={handleClose}>
			<DialogTitle id="new-agent-dialog-title" onClose={handleClose}>
				{type === 'new' ? 'New' : 'Edit'} Agent
			</DialogTitle>
			{loading ? (
