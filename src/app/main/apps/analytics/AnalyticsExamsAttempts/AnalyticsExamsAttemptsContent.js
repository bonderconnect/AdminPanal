...
import { getExamsAttempts, setParams } from '../store/examsAttemptsSlice';
import AnalyticsExamsAttemptsContentSearch from './AnalyticsExamsAttemptsContentSearch';
...
const AnalyticsVideosWatchContent = () => {
	...
	const meta = useSelector(state => state.analyticsApp.examsAttempts.meta);
	const { loading, data } = useSelector(state => state.analyticsApp.examsAttempts.list);
			...
			data.forEach((item, index) => {
				const rowItem = {
					...
					cells: [
						...
						{
							id: 'total_distinct_attempts',
							value: item.total_distinct_attempts,
		...
		<div className="flex flex-col">
			<Paper className="w-full rounded-20 shadow overflow-hidden">
				<div className="flex items-center justify-between m-20 h-64">
					<div>
						<AnalyticsExamsAttemptsContentSearch />
					</div>
												...
												return cell.id === 'user_avatar' ? (
													<TableCell key={cell.id} component="th" scope="row">
														<div className="flex items-center">
															...
															<div className="flex flex-col">
																{row.item.phone ? <span>{row.item.phone}</span> : null}
																{row.item.email ? <span>{row.item.email}</span> : null}
															</div>
														</div>
													</TableCell>
												) : (
											...
											<TableCell component="th" scope="row">
												<div className="flex items-center">
													<Button
														...
														to={`/apps/users/user/${row.id}/analytics/exams`}
														variant="outlined"
														size="small"
