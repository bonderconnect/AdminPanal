...
function CategoriesMachineryContent(props) {
	const dispatch = useDispatch();
	const status = useSelector(state => state.categoriesMachineryApp.categoriesMachinery.status);
	const categories = useSelector(state => state.categoriesMachineryApp.categoriesMachinery.categories);
	...
	const uniqueMachinerys = servicesWithImageURL?.filter(item => {
	...
	useEffect(() => {
		if (status === 'idle') {
			dispatch(getCategoriesMachinery(['machinery']));
		}
	}, [dispatch, status]);
		...
		<FuseAnimate animation="transition.slideUpIn" delay={300}>
			<div className="flex px-16">
				<Table>
					...
					<TableBody>
						{uniqueMachinerys?.map(item => {
							return (
								<TableRow
									key={item.category_id}
									hover
									onClick={() => dispatch(openEditCategoriesMachineryDialog(item))}
															...
															<>
																...
																<DialogContent>
																	<DialogContentText id="alert-dialog-description">
																		...
																		retrieve the Machinery category's data within 30
...
export default CategoriesMachineryContent;
