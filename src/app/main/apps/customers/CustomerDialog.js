...
import { closeDialog, createCustomer, updateCustomer } from './store/customerDialog';
...
function CustomerDialog(props) {
	...
	const {
		...
		errors: storeErrors
	} = useSelector(({ customersApp }) => customersApp.customerDialog);
	...
	const onSubmit = (data, e) => {
		if (type === 'new') {
			dispatch(createCustomer({ form: data, callback }));
		} else {
			dispatch(updateCustomer({ form: data, callback }));
		}
	};
	...
	return (
		<Dialog fullWidth maxWidth="xs" aria-labelledby="new-customer-dialog-title" open={open} onClose={handleClose}>
			<DialogTitle id="new-customer-dialog-title" onClose={handleClose}>
				{type === 'new' ? 'New' : 'Edit'} Customer
			</DialogTitle>
			{loading ? (
