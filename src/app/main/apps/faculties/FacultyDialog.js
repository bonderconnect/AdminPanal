...
import { closeDialog, createFaculty, updateFaculty } from './store/facultyDialog';
import { getFaculties } from './store/faculties';

function FacultyDialog(props) {
	...
	const {
		...
		errors: storeErrors
	} = useSelector(({ facultiesApp }) => facultiesApp.facultyDialog);
	...
	const callback = () => {
		if (type === 'new') {
			dispatch(showMessage({ message: 'Faculty created successfully' }));
		} else {
			dispatch(showMessage({ message: 'Faculty updated successfully' }));
		}
	...
	const onSubmit = (data, e) => {
		if (type === 'new') {
			dispatch(createFaculty({ form: data, callback }));
		} else {
			dispatch(updateFaculty({ form: data, callback }));
		}
	};
	...
	return (
		<Dialog fullWidth maxWidth="xs" aria-labelledby="new-student-dialog-title" open={open} onClose={handleClose}>
			<DialogTitle id="new-student-dialog-title" onClose={handleClose}>
				{type === 'new' ? 'New' : 'Edit'} Faculty
			</DialogTitle>
			{loading ? (
