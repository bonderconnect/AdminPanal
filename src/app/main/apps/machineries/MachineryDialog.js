...
import { closeDialog, createMachinery, updateMachinery } from './store/machineryDialog';
import { getMachineries } from './store/machineries';

function MachineryDialog(props) {
	...
	const {
		...
		errors: storeErrors
	} = useSelector(({ machineriesApp }) => machineriesApp.machineryDialog);
	...
	const callback = () => {
		if (type === 'new') {
			dispatch(showMessage({ message: 'Machinery created successfully' }));
		} else {
			dispatch(showMessage({ message: 'Machinery updated successfully' }));
		}
		dispatch(getMachineries());
	};

	const onSubmit = (data, e) => {
		if (type === 'new') {
			dispatch(createMachinery({ form: data, callback }));
		} else {
			dispatch(updateMachinery({ form: data, callback }));
		}
	};
	...
	return (
		<Dialog fullWidth maxWidth="xs" aria-labelledby="new-machinery-dialog-title" open={open} onClose={handleClose}>
			<DialogTitle id="new-machinery-dialog-title" onClose={handleClose}>
				{type === 'new' ? 'New' : 'Edit'} Machinery
			</DialogTitle>
			{loading ? (
...
export default MachineryDialog;
