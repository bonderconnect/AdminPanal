...
import MachineriesTable from './MachineriesTable';
import { selectMachineries, getMachineries, setParams } from '../store/machineries';
import { openEditMachineryDialog } from '../store/machineryDialog';
...
function MachineriesContent() {
	const classes = useStyles();
	const dispatch = useDispatch();
	const machineries = useSelector(selectMachineries);
	// const searchText = useSelector(({ usersApp }) => usersApp.users.searchText);
	const { params, loading, meta } = useSelector(({ machineriesApp }) => machineriesApp.machineries);
	const count = meta.totalCount;

	useDeepCompareEffect(() => {
		dispatch(getMachineries(params));
	}, [dispatch, params]);
	...
	const columns = useMemo(
		() => [
			...
			{
				Header: 'Machinery meta details',
				id: 'machinery_meta_details',
					...
					<div className="flex flex-col justify-center">
						...
						<div className="mb-4">
							...
							<span className={clsx(classes.columnContentValue, classes.tableStrictWidthContent)}>
								{helpers.truncate(row.original.company_address, { limit: 4, elipses: true }) || null}
							</span>
						</div>
					</div>
				)
					...
					<div className="flex items-end justify-end">
						<div className="flex items-center align-center">
							<Button
								onClick={ev => {
									ev.stopPropagation();
									dispatch(openEditMachineryDialog(row.original.created_user_id));
								}}
								variant="contained"
								size="small"
								color="primary"
											...
											children: (
												<>
													<DialogTitle id="alert-dialog-title">
														Are you sure want to delete this machinery?
													</DialogTitle>
													<DialogContent>
		...
		return (
			<div className="flex flex-1 items-center justify-center h-full">
				<Typography color="textSecondary" variant="h5">
					There are no machineries to show !
				</Typography>
			</div>
		);
	}
...
export default MachineriesContent;
