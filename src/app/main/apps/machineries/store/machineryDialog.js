...
export const createMachinery = createAsyncThunk(
	'machineriesApp/machineryDialog/createMachinery',
...
export const updateMachinery = createAsyncThunk(
	'machineriesApp/machineryDialog/updateMachinery',
	async (payload, { dispatch, getState, rejectWithValue }) => {
		const { form } = payload;
		const { userId } = getState().machineriesApp.machineryDialog.data;
...
export const openEditMachineryDialog = createAsyncThunk(
	'machineriesApp/machineryDialog/openEditMachineryDialog',
	async userId => {
...
const machineryDialogueSlice = createSlice({
	name: 'machineriesApp/machineryDialog',
	initialState,
	reducers: {
		openNewMachineryDialog: () => {
	...
	extraReducers: {
		[createMachinery.pending]: (state, action) => {
			state.submitting = true;
		},
		[createMachinery.rejected]: (state, action) => {
			if (action.payload && action.payload.errors) {
				state.errors = action.payload.errors;
			}
			state.submitting = false;
		},
		[createMachinery.fulfilled]: (state, action) => {
			state.open = false;
		},
		[openEditMachineryDialog.pending]: (state, action) => {
		...
		[openEditMachineryDialog.fulfilled]: (state, action) => {
			state.form = action.payload;
			state.loading = false;
		},
		[updateMachinery.rejected]: (state, action) => {
			if (action.payload && action.payload.errors) {
				state.errors = action.payload.errors;
			}
			state.submitting = false;
		},
		[updateMachinery.pending]: state => {
			state.submitting = true;
		},
		[updateMachinery.fulfilled]: state => {
			state.submitting = true;
			state.open = false;
		}
	}
...
export const { openNewMachineryDialog, closeDialog } = machineryDialogueSlice.actions;

export default machineryDialogueSlice.reducer;
