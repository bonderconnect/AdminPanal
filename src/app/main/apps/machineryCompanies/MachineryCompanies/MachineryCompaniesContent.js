...
import MachineryCompaniesTable from './MachineryCompaniesTable';
import { selectMachineryCompanies, getMachineryCompanies, setParams } from '../store/machineryCompanies';
import { openEditMachineryCompanyDialog } from '../store/machineryCompanyDialog';
...
function MachineryCompaniesContent() {
	const classes = useStyles();
	const dispatch = useDispatch();
	const machineryCompanies = useSelector(selectMachineryCompanies);
	// const searchText = useSelector(({ usersApp }) => usersApp.users.searchText);
	const { params, loading, meta } = useSelector(
		({ machineryCompaniesApp }) => machineryCompaniesApp.machineryCompanies
	);
	const count = meta.totalCount;

	useDeepCompareEffect(() => {
		dispatch(getMachineryCompanies(params));
	}, [dispatch, params]);
					...
					<div className="flex items-end justify-end">
						<div className="flex items-center align-center">
							<Button
								onClick={ev => {
									ev.stopPropagation();
									dispatch(openEditMachineryCompanyDialog(row.original.id));
								}}
								variant="contained"
								size="small"
								color="primary"
	...
	if (!loading && machineryCompanies.length === 0) {
		return (
			<div className="flex flex-1 items-center justify-center h-full">
				<Typography color="textSecondary" variant="h5">
					There are no Machinery company profiles to show !
				</Typography>
			</div>
		);
	}
	...
	return (
		<MachineryCompaniesTable
			loading={loading}
			columns={columns}
			data={machineryCompanies}
			goToPage={goToPage}
...
export default MachineryCompaniesContent;
