...
export const createMachineryCompany = createAsyncThunk(
	'machineryCompaniesApp/machineryCompanyDialog/createMachineryCompany',
...
export const updateMachineryCompany = createAsyncThunk(
	'machineryCompaniesApp/machineryCompanyDialog/updateMachineryCompany',
	async (payload, { dispatch, getState, rejectWithValue }) => {
		const { form } = payload;
		const { userId } = getState().machineryCompaniesApp.machineryCompanyDialog.data;
...
export const openEditMachineryCompanyDialog = createAsyncThunk(
	'machineryCompaniesApp/machineryCompanyDialog/openEditMachineryCompanyDialog',
	async userId => {
...
const machineryCompanyDialogueSlice = createSlice({
	name: 'machineryCompaniesApp/machineryCompanyDialog',
	initialState,
	reducers: {
		openNewMachineryCompanyDialog: () => {
	...
	extraReducers: {
		[createMachineryCompany.pending]: (state, action) => {
			state.submitting = true;
		},
		[createMachineryCompany.rejected]: (state, action) => {
			if (action.payload && action.payload.errors) {
				state.errors = action.payload.errors;
			}
			state.submitting = false;
		},
		[createMachineryCompany.fulfilled]: (state, action) => {
			state.open = false;
		},
		[openEditMachineryCompanyDialog.pending]: (state, action) => {
		...
		[openEditMachineryCompanyDialog.fulfilled]: (state, action) => {
			state.form = action.payload;
			state.loading = false;
		},
		[updateMachineryCompany.rejected]: (state, action) => {
			if (action.payload && action.payload.errors) {
				state.errors = action.payload.errors;
			}
			state.submitting = false;
		},
		[updateMachineryCompany.pending]: state => {
			state.submitting = true;
		},
		[updateMachineryCompany.fulfilled]: state => {
			state.submitting = true;
			state.open = false;
		}
	}
...
export const { openNewMachineryCompanyDialog, closeDialog } = machineryCompanyDialogueSlice.actions;

export default machineryCompanyDialogueSlice.reducer;
