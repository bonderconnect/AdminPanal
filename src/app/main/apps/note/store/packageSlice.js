...
export const getPackages = createAsyncThunk('noteApp/packages/getPackages', async () => {
	const response = await axios
		.get('/packages', { params: { status_value: 1 } })
...
export const { selectAll: selectPackages, selectById: selectPackageById } = packagesAdapter.getSelectors(
	state => state.noteApp.packages
);

const packagesSlice = createSlice({
	name: 'noteApp/packages',
