...
import { closeDialog, createProduct, updateProduct } from './store/productDialog';
...
function ProductDialog(props) {
	...
	const {
		...
		errors: storeErrors
	} = useSelector(({ productsApp }) => productsApp.productDialog);
	...
	const onSubmit = (data, e) => {
		if (type === 'new') {
			dispatch(createProduct({ form: data, callback }));
		} else {
			dispatch(updateProduct({ form: data, callback }));
		}
	};
	...
	return (
		<Dialog fullWidth maxWidth="xs" aria-labelledby="new-product-dialog-title" open={open} onClose={handleClose}>
			<DialogTitle id="new-product-dialog-title" onClose={handleClose}>
				{type === 'new' ? 'New' : 'Edit'} Product
			</DialogTitle>
			{loading ? (
