...
import ServiceCompaniesTable from './ServiceCompaniesTable';
import { selectServiceCompanies, getServiceCompanies, setParams } from '../store/serviceCompanies';
import { openEditServiceCompanyDialog } from '../store/serviceCompanyDialog';
...
function ServiceCompaniesContent() {
	const classes = useStyles();
	const dispatch = useDispatch();
	const serviceCompanies = useSelector(selectServiceCompanies);
	// const searchText = useSelector(({ usersApp }) => usersApp.users.searchText);
	const { params, loading, meta } = useSelector(({ serviceCompaniesApp }) => serviceCompaniesApp.serviceCompanies);
	const count = meta.totalCount;

	useDeepCompareEffect(() => {
		dispatch(getServiceCompanies(params));
	}, [dispatch, params]);
					...
					<div className="flex items-end justify-end">
						<div className="flex items-center align-center">
							<Button
								onClick={ev => {
									ev.stopPropagation();
									dispatch(openEditServiceCompanyDialog(row.original.id));
								}}
								variant="contained"
								size="small"
								color="primary"
	...
	if (!loading && serviceCompanies.length === 0) {
		return (
			<div className="flex flex-1 items-center justify-center h-full">
				<Typography color="textSecondary" variant="h5">
					There are no Service company profiles to show !
				</Typography>
			</div>
		);
	}
...
export default ServiceCompaniesContent;
