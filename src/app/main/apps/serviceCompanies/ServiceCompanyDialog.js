...
import { closeDialog, createServiceCompany, updateServiceCompany } from './store/serviceCompanyDialog';
import { getServiceCompanies } from './store/serviceCompanies';

function ServiceCompanyDialog(props) {
	...
	const {
		...
		errors: storeErrors
	} = useSelector(({ serviceCompaniesApp }) => serviceCompaniesApp.serviceCompanyDialog);
	...
	const callback = () => {
		if (type === 'new') {
			dispatch(showMessage({ message: 'Service company created successfully' }));
		} else {
			dispatch(showMessage({ message: 'Service company updated successfully' }));
		}
		dispatch(getServiceCompanies());
	};

	const onSubmit = (data, e) => {
		if (type === 'new') {
			dispatch(createServiceCompany({ form: data, callback }));
		} else {
			dispatch(updateServiceCompany({ form: data, callback }));
		}
	};
	...
	return (
		<Dialog
			fullWidth
			maxWidth="xs"
			aria-labelledby="new-serviceCompany-dialog-title"
			open={open}
			onClose={handleClose}
		>
			<DialogTitle id="new-serviceCompany-dialog-title" onClose={handleClose}>
...
export default ServiceCompanyDialog;
