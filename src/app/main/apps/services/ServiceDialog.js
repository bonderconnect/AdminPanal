...
import { closeDialog, createService, updateService } from './store/serviceDialog';
...
function ServiceDialog(props) {
	...
	const {
		...
		errors: storeErrors
	} = useSelector(({ servicesApp }) => servicesApp.serviceDialog);
	...
	const onSubmit = (data, e) => {
		if (type === 'new') {
			dispatch(createService({ form: data, callback }));
		} else {
			dispatch(updateService({ form: data, callback }));
		}
	};
	...
	return (
		<Dialog fullWidth maxWidth="xs" aria-labelledby="new-service-dialog-title" open={open} onClose={handleClose}>
			<DialogTitle id="new-service-dialog-title" onClose={handleClose}>
				{type === 'new' ? 'New' : 'Edit'} Service
			</DialogTitle>
			{loading ? (
