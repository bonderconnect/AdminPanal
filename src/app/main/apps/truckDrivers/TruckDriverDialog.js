...
import { closeDialog, createTruckDriver, updateTruckDriver } from './store/truckDriverDialog';
import { getTruckDrivers } from './store/truckDrivers';

function TruckDriverDialog(props) {
	...
	const {
		...
		errors: storeErrors
	} = useSelector(({ truckDriversApp }) => truckDriversApp.truckDriverDialog);
	...
	const onSubmit = (data, e) => {
		if (type === 'new') {
			dispatch(createTruckDriver({ form: data, callback }));
		} else {
			dispatch(updateTruckDriver({ form: data, callback }));
		}
	};
	...
	return (
		<Dialog
			fullWidth
			maxWidth="xs"
			aria-labelledby="new-truckdriver-dialog-title"
			open={open}
			onClose={handleClose}
		>
			<DialogTitle id="new-truckdriver-dialog-title" onClose={handleClose}>
...
export default TruckDriverDialog;
