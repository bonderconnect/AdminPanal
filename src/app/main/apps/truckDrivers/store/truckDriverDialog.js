...
export const createTruckDriver = createAsyncThunk(
	'truckDriversApp/truckDriverDialog/createTruckDriver',
...
export const updateTruckDriver = createAsyncThunk(
	'truckDriversApp/truckDriverDialog/updateTruckDriver',
	async (payload, { dispatch, getState, rejectWithValue }) => {
		const { form } = payload;
		const { userId } = getState().truckDriversApp.truckDriverDialog.data;
...
export const openEditTruckDriverDialog = createAsyncThunk(
	'truckDriversApp/truckDriverDialog/openEditTruckDriverDialog',
	async userId => {
...
const truckDriverDialogueSlice = createSlice({
	name: 'truckDriversApp/truckDriverDialog',
	initialState,
	reducers: {
		openNewTruckDriverDialog: () => {
	...
	extraReducers: {
		[createTruckDriver.pending]: (state, action) => {
			state.submitting = true;
		},
		[createTruckDriver.rejected]: (state, action) => {
			if (action.payload && action.payload.errors) {
				state.errors = action.payload.errors;
			}
			state.submitting = false;
		},
		[createTruckDriver.fulfilled]: (state, action) => {
			state.open = false;
		},
		[openEditTruckDriverDialog.pending]: (state, action) => {
		...
		[openEditTruckDriverDialog.fulfilled]: (state, action) => {
			state.form = action.payload;
			state.loading = false;
		},
		[updateTruckDriver.rejected]: (state, action) => {
			if (action.payload && action.payload.errors) {
				state.errors = action.payload.errors;
			}
			state.submitting = false;
		},
		[updateTruckDriver.pending]: state => {
			state.submitting = true;
		},
		[updateTruckDriver.fulfilled]: state => {
			state.submitting = true;
			state.open = false;
		}
	}
...
export const { openNewTruckDriverDialog, closeDialog } = truckDriverDialogueSlice.actions;

export default truckDriverDialogueSlice.reducer;
