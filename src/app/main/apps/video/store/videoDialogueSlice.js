...
export const addVideo = createAsyncThunk('videoApp/videoDialogue/addVideo', async (payload, { dispatch, getState }) => {
			...
			if (video.user_restriction) {
				return new Promise(resolve => {
					...
					const {
						...
						unsavedSelection
					} = state.videoApp.videoDialogue.userRestrictionDialogue;
...
export const updateVideo = createAsyncThunk(
	'videoApp/videoDialogue/updateVideo',
	async (payload, { dispatch, getState }) => {
		...
		if (video.user_restriction || video.remove_user_restriction) {
			const state = getState();
			const { userRestrictionDialogue } = state.videoApp.videoDialogue;
...
export const openEditVideoDialog = createAsyncThunk(
	'videoApp/videoDialogue/openEditVideoDialog',
...
export const userRestrictionSearchUsers = createAsyncThunk(
	'videoApp/videoDialogue/userRestrictionSearchUsers',
	async (searchText, { dispatch, getState, rejectWithValue }) => {
		const state = getState();
		const { unsavedSelectionUserIds } = state.videoApp.videoDialogue.userRestrictionDialogue;
		const { CancelToken } = axiosDefault;
		axios.isCancel = axiosDefault.isCancel;
		const cancelToken = new CancelToken(c => {
			// An executor function receives a cancel function as a parameter
			...
			window.axiosCancelReference['videoApp/videoDialogue/userRestrictionSearchUsers'] = c;
		});
...
const videoDialogueSlice = createSlice({
	...
	reducers: {
		...
		cancelFileUpload: state => {
			if (window.axiosCancelReference && window.axiosCancelReference['videoApp/videoDialogue/updateVideo']) {
				window.axiosCancelReference['videoApp/videoDialogue/updateVideo']();
			}
			state.upload = {
				inProgress: false,
				percentage: null,
